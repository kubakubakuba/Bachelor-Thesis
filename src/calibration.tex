...
\begin{figure}[H]
	...
	\caption{
		Lenses used during the calibration, a $93.5$ degree lens in \reffig{fig:lens_1} and Entaniya $280$ degree lens in \reffig{fig:lens_2}.
  }
	\label{fig:lenses}
\end{figure}


In this work a calibration method proposed by Scaramuzza et al. \cite{scaramuzzacalibration} is used to calibrate all used equipment. The implementation used for calibration is a Python library py-OCamCalib \footnote{py-OCamCalib is available at \url{https://github.com/jakarto3d/py-OCamCalib}}.%TODO: maybe cite the github repo
...
We can now fit the encompassing ellipse, which can be formulated as an optimization problem \refeq{eq:ellipse_opt} of minimizing the sum of point distances, that lie outside of the
...
The calibration results can be seen on \reffig{fig:calib_ent_res}, with the visualization on \reffig{fig:calib_ent_viz} and the perspective projection on
\reffig{fig:calib_ent_proj}.

%The output can be seen in \reffig{fig:calib_ent}
%\begin{figure}[H]
%	\centering
